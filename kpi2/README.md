...
3. Drag fields onto the encoding tiles that appear on the Marks card:
   ...
   - **Trend date** (optional): a date (e.g. `MONTH(Order Placed Date)`)
     → the x-axis of the sparkline.
   - **Trend value** (optional): the measure plotted as the line, e.g.
     `Total Participation` → lets the line show full history independently of
     the big number. If left empty, the line falls back to the Value field.

### Trend line: which measures work

The big number and the sparkline share one worksheet, so they share one grain.
That has a consequence for the **Value** number when a Trend date is present:

- **Additive measures (SUM)** — e.g. `YTD This Year` / `Total Participation`:
  fully supported. The number reconstitutes correctly even with a date on the
  sheet, and the **Trend value** tile lets the line show full history while the
  number stays YTD. This is the intended setup for the Participation cards.
- **Non-additive measures (COUNTD, AVG, ratios)** — e.g. `YTD Active LOs` or
  `Avg. Participant/LO`: do **not** put a Trend date on the same card, because a
  per-date breakout can't be summed back into a correct distinct count or
  average. For those cards, either skip the line, or build the sparkline as a
  separate native worksheet and float it beneath the card.
